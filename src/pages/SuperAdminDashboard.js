...
function poolBadge(pool) {
  const m={riasec:"bg-grn",bigfive:"bg-blu",aptitude:"bg-acc"};
          ...
          <div className="apt-grid">
            {Object.entries(data.avgAptitudeScores).map(([subj,pct],i)=>(
              <div key={subj} className="apt-tile">
                ...
                <div className="apt-pct" style={{color:STRAND_COLORS[i]}}>{pct}%</div>
              </div>
            ))}
          </div>
        </div>
...
function QuestionModal({pool,editDoc,onClose,onSaved}) {
  ...
  const [form,setForm]=useState(()=>{
    ...
    return {pool,text:"",subcategory:pool==="riasec"?RIASEC_SUBS[0]:pool==="bigfive"?BIGFIVE_SUBS[0]:"",reverse_scored:false,subject:"math",topic:APT_TOPICS["math"][0],difficulty:"easy",options:{A:"",B:"",C:"",D:""},correct_answer:"A"};
  });
  ...
  return (
    <div className="mo" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mo-box">
        ...
        {form.pool==="bigfive"&&(
          <>
            <div className="fr"><label>Trait</label>
              <select className="sel" value={form.subcategory||""} onChange={e=>set("subcategory",e.target.value)}>
                {BIGFIVE_SUBS.map(s=><option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div className="fr" style={{flexDirection:"row",alignItems:"center",gap:9}}>
              <input type="checkbox" id="rev" checked={!!form.reverse_scored} onChange={e=>set("reverse_scored",e.target.checked)} style={{accentColor:"var(--acc)",width:15,height:15}}/>
              <label htmlFor="rev" style={{textTransform:"none",letterSpacing:0,fontSize:13,color:"var(--t1)",cursor:"pointer",fontWeight:400}}>Reverse scored</label>
            </div>
          </>
        )}
        {form.pool==="aptitude"&&(
...
function QuestionsPanel() {
  ...
  const loadPoolStats=useCallback(async()=>{
    const stats={};
    await Promise.all(["riasec","bigfive","aptitude"].map(async p=>{
      try {
  ...
  const POOL_COLORS={riasec:STRAND_COLORS[2],bigfive:STRAND_COLORS[1],aptitude:STRAND_COLORS[0]};
                    ...
                    <tr key={q.id}>
                      ...
                      <td>
                        ...
                        {pool==="bigfive"&&q.reverse_scored&&(
                          <span className="bd2 bg-ora" style={{marginTop:4,display:"inline-block",fontSize:9.5}}>reverse</span>
                        )}
                      </td>
                      <td>
            ...
            <div key={i} className="res-card">
              ...
              {r.scores?.aptitude_pct&&(
                <div className="score-grid">
                  {Object.entries(r.scores.aptitude_pct).map(([s,v],i)=>(
...
const SUBTITLES={
  ...
  questions:   "Manage assessment pool questions with bulk actions",
